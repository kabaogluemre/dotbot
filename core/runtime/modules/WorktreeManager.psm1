...
function New-TaskWorktree {
    ...
    try {
        ...
        # 8. .mcp.json — Claude MCP discovery reads this from cwd. The task worktree is
        # Claude's cwd during task execution, so .mcp.json must be reachable there.
        # If .mcp.json is tracked in git, the worktree already has it (checked out from
        # base branch). Only materialize a copy when the worktree lacks it (project uses
        # an untracked/gitignored .mcp.json — common when `.mcp.json` contains secrets
        # or is generated by init). We plain-copy (not hardlink) so agent edits to the
        # worktree's .mcp.json don't leak back to the project root's version.
        $mainMcpJson = Join-Path $ProjectRoot ".mcp.json"
        $worktreeMcpJson = Join-Path $worktreePath ".mcp.json"
        if ((Test-Path $mainMcpJson) -and (-not (Test-Path $worktreeMcpJson))) {
            try {
                Copy-Item -Path $mainMcpJson -Destination $worktreeMcpJson -Force -ErrorAction Stop
            } catch {
                Write-BotLog -Level Warn -Message "Could not materialize .mcp.json in worktree — Claude MCP discovery may fail" -Exception $_
            }
        }
