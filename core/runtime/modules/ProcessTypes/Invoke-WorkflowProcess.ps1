...
# ── Shared / feature branch resolution from workflow manifest ──────────────
# Two opt-in modes (mutually exclusive; feature mode wins if both are set):
#   shared_feature_branch: true  → per-task worktrees fork from ONE auto-named
#       feature branch and squash-merge back into it; a final task opens a PR to
#       main. The branch name is generated by the framework. ($featureBranch)
#   shared_branch: "<template>"  → legacy: all tasks reuse ONE worktree/branch
#       and commit directly to it; the template is resolved from input. ($sharedBranch)
$sharedBranch  = $null
$featureBranch = $null
$workflowManifest = Get-ActiveWorkflowManifest -BotRoot $botRoot

# Robustly resolve an issue number from the kickstart prompt (URL / #N / trailing
# int). A bare \D-strip concatenates every digit, so an issue URL ending in
# ".../repo-2026/issues/24" would wrongly yield "202624".
$promptFile  = Join-Path $botRoot ".control\launchers\workflow-launch-prompt.txt"
$issueNumber = $null
if (Test-Path $promptFile) {
    $promptText = (Get-Content $promptFile -Raw -ErrorAction SilentlyContinue)
    if ($promptText) {
        $promptText = $promptText.Trim()
        if     ($promptText -match 'github\.com/[^/\s]+/[^/\s]+/(?:issues|pull)/(\d+)') { $issueNumber = $Matches[1] }
        elseif ($promptText -match '#(\d+)')                                            { $issueNumber = $Matches[1] }
        elseif ($promptText -match '(?:^|\s)(\d+)\s*$')                                  { $issueNumber = $Matches[1] }
    }
}

# Decide mode + the base (un-suffixed) branch name.
$sfbVal = if ($workflowManifest) { $workflowManifest.shared_feature_branch } else { $null }
$isFeatureMode = ($sfbVal -eq $true) -or ("$sfbVal" -eq 'true')
$branchBaseName = $null
$branchAssignTo = $null
# Branch the feature branch forks FROM. $null → repo main/master (default,
# resolved downstream by New-SharedFeatureBranch).
$featureBaseBranch = $null
if ($isFeatureMode) {
    $wfLabel = if ($Workflow) { $Workflow } else { 'run' }
    $branchBaseName = if ($issueNumber) { "feature/issue-$issueNumber" } else { "feature/$wfLabel" }
    $branchAssignTo = 'feature'

    # Opt-in override for the fork base via the manifest's `feature_branch_base`:
    #   "current"  → fork from whatever branch the project is on right now. Needed
    #                when .bot/ (framework + dotbot-registered .mcp.json) lives on a
    #                non-main branch — worktrees only carry what is committed on the
    #                base, so forking from main would yield a worktree with no .bot/.
    #   "<branch>" → fork from a specific named branch.
    #   absent/"main"/"master" → default main/master behaviour.
    $featureBaseOpt = if ($workflowManifest) { "$($workflowManifest.feature_branch_base)".Trim() } else { "" }
    if ($featureBaseOpt -eq 'current') {
        $headBranch = (git -C $projectRoot rev-parse --abbrev-ref HEAD 2>$null)
        if ($headBranch) { $headBranch = "$headBranch".Trim() }
        if ($headBranch -and $headBranch -ne 'HEAD') {
            $featureBaseBranch = $headBranch
            Write-Status "feature_branch_base=current → forking feature branch from '$featureBaseBranch'" -Type Info
        } else {
            Write-Status "feature_branch_base=current but project is in detached HEAD — falling back to main/master" -Type Warn
        }
    } elseif ($featureBaseOpt -and $featureBaseOpt -notin @('main', 'master')) {
        $featureBaseBranch = $featureBaseOpt
    }
} elseif ($workflowManifest -and $workflowManifest.shared_branch) {
    $resolved = $workflowManifest.shared_branch
    if ($issueNumber) { $resolved = $resolved -replace '\{input\.issue_number\}', $issueNumber }
    if ($resolved -and $resolved -notmatch '\{') {
        $branchBaseName = $resolved
        $branchAssignTo = 'shared'
    } else {
...
if ($branchBaseName) {
    # Append a per-run suffix so reruns get a fresh branch/worktree/PR. Persisted
    # under .control/workflow-runs/{workflow}.json so resumes (new procId) reuse
    # the branch; server.ps1 deletes this file on a `rerun: fresh` launch.
    ...
    if (Test-Path $runStateFile) {
        try {
            ...
            if ($state -and $state.base -eq $branchBaseName -and $state.shared_branch) {
                $resolvedBranch = [string]$state.shared_branch
                $wasResumed = $true
            }
    ...
    if (-not $resolvedBranch) {
        ...
        $resolvedBranch = "$branchBaseName-$runSuffix"
        if (-not (Test-Path $runStateDir)) { New-Item -Path $runStateDir -ItemType Directory -Force | Out-Null }
        ([ordered]@{
            base          = $branchBaseName
            shared_branch = $resolvedBranch
            created_at    = (Get-Date).ToUniversalTime().ToString("o")
            proc_id       = $procId
        }) | ConvertTo-Json -Depth 3 | Set-Content -Path $runStateFile -Encoding UTF8
    }

    $modeWord = if ($wasResumed) { "resumed" } else { "active" }
    if ($branchAssignTo -eq 'feature') {
        $featureBranch = $resolvedBranch
        # Create the feature branch off the resolved base ($featureBaseBranch, or
        # main/master when null) now so task worktrees can fork from it.
        $fbParams = @{ ProjectRoot = $projectRoot; FeatureBranch = $featureBranch }
        if ($featureBaseBranch) { $fbParams['BaseBranch'] = $featureBaseBranch }
        $fbResult = New-SharedFeatureBranch @fbParams
        if ($fbResult.success) {
            Write-Status "Feature-branch mode ($modeWord): $featureBranch — $($fbResult.message)" -Type Info
            Write-ProcessActivity -Id $procId -ActivityType "text" -Message "Feature-branch mode $($modeWord): $($featureBranch) ($($fbResult.message))"
        } else {
            Write-Status "Feature-branch mode: could not create '$featureBranch' — $($fbResult.message)" -Type Warn
            Write-ProcessActivity -Id $procId -ActivityType "text" -Message "Feature branch creation failed: $($fbResult.message)"
        }
    } else {
        $sharedBranch = $resolvedBranch
        Write-Status "Shared branch mode ($modeWord): $sharedBranch" -Type Info
        Write-ProcessActivity -Id $procId -ActivityType "text" -Message "Shared branch mode $($modeWord): $($sharedBranch)"
    }
}
...
"@
        ...
        if ($skipWorktree) {
            ...
            if ($wtInfo -and (Test-Path $wtInfo.worktree_path)) {
                ...
                # Worktree base depends on mode:
                #   feature mode   → each task gets its OWN branch forked FROM the
                #                    shared feature branch (-BaseBranch); on
                #                    completion Complete-TaskWorktree squash-merges
                #                    it back into that feature branch.
                #   legacy shared  → all tasks reuse ONE branch/worktree (-BranchName).
                #   normal         → per-task branch forked from main/master.
                $wtParams = @{
                    TaskId      = $task.id
                    TaskName    = $task.name
                ...
                if ($featureBranch) {
                    $wtParams['BaseBranch'] = $featureBranch
                } elseif ($sharedBranch) {
                    $wtParams['BranchName'] = $sharedBranch
                }
                $wtResult = New-TaskWorktree @wtParams
                if ($wtResult.success) {
...
"@
        ...
        while ($attemptNumber -le $maxRetriesPerTask) {
            ...
            try {
                ...
                # land on the task/shared branch instead of project root (#314).
        ...
        if ($taskParked) {
            ...
            # Shared branch mode: just commit + push (the branch stays open and is
            # merged via PR after all tasks complete). Otherwise squash-merge to main.
            if ($worktreePath -and $sharedBranch) {
                ...
                # Squash-merge the task branch into its recorded base — main/master
                # in normal mode, or the shared feature branch in feature mode.
                $mergeTargetLabel = if ($featureBranch) { $featureBranch } else { "main" }
